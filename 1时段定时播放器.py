...
class WarmPlayer(QMainWindow):
    def __init__(self):
        super(WarmPlayer, self).__init__()
        ...
        edits = [self.inp_st_h, self.inp_st_m, self.inp_st_s,
                 self.inp_ed_h, self.inp_ed_m, self.inp_ed_s]
        for ed in edits:
            ed.editingFinished.connect(self.save_current_config)

        self.time_thread = threading.Thread(target=self.time_detect_loop)
    ...
    def time_detect_loop(self):
        while self.running:
            ...
            # 手动锁定时，定时线程禁止自动开播
            if in_slot and not self.is_playing and len(self.music_list) > 0 and not self.block_auto_play:
                self.start_play()
            if not in_slot and self.is_playing:
                self.stop_play_by_time()
            time.sleep(1)
    ...
    def load_music_folder(self, path):
        ...
        self.lab_status.setText("载入{}首音频".format(len(files)))
        if len(self.music_list) > 1:
            self.lab_next.setText("下一首：" + os.path.basename(self.music_list[1]))
        else:
            self.lab_next.setText("下一首：无")
    ...
    def play_one(self):
        if not self.music_list or self.cur_idx >= len(self.music_list):
            return
        self.kill_ff()
        path = self.music_list[self.cur_idx]
        ff_cmd = "ffplay.exe" if sys.platform == "win32" else "ffplay"
        cmd = [ff_cmd, "-nodisp", "-autoexit", "-volume", str(self.cur_volume), path]
        try:
            # Windows隐藏CMD黑窗口核心代码
            startupinfo = None
            ...
            self.lab_now.setText("当前曲目：{}".format(name))
            next_idx = self.cur_idx + 1 if (self.cur_idx + 1 < len(self.music_list)) else 0
            self.lab_next.setText("下一首：{}".format(os.path.basename(self.music_list[next_idx])))
        except Exception:
            self.lab_status.setText("错误：未找到 ffplay.exe")
    ...
    def play_loop_func(self):
        while self.is_playing and self.running:
            ...
            self.play_one()
            while self.ff_proc is not None and self.ff_proc.poll() is None and self.is_playing:
                time.sleep(0.25)
            ...
            if self.cur_idx >= len(self.music_list):
                if self.loop_flag:
                    ...
                    self.stop_play_by_finish()
                    break

    def start_play(self):
        # 手动点击开始，解除锁定
        self.block_auto_play = False
        if self.is_playing or len(self.music_list) == 0:
            return
        self.is_playing = True
        t = threading.Thread(target=self.play_loop_func)
        t.daemon = True
        t.start()
        self.lab_status.setText("正在播放")

    # 定时到点自动停止（不锁自动）
    def stop_play_by_time(self):
        self.is_playing = False
        self.kill_ff()
        self.lab_status.setText("定时结束已停止")

    # 播放完毕自动停止（不锁自动）
    def stop_play_by_finish(self):
        self.is_playing = False
        self.kill_ff()
        self.lab_status.setText("播放完毕（非循环模式）")

    # 手动按钮停止：锁定自动播放（核心修复）
    def stop_play(self):
        self.is_playing = False
        self.block_auto_play = True
        self.kill_ff()
        self.lab_status.setText("手动停止，已锁定自动播放")
